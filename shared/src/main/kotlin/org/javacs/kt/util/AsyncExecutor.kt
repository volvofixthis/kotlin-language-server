...
class AsyncExecutor {
    ...
    fun execute(task: () -> Unit): CompletableFuture<Void> = CompletableFuture.runAsync(Runnable(task), workerThread)

    fun <R> compute(task: () -> R): CompletableFuture<R> = CompletableFuture.supplyAsync(Supplier(task), workerThread)
